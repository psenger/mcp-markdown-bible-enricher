...
describe("config", () => {
  ...
  describe("loadConfig", () => {
    ...
    test("should load custom Obsidian format", () => {
      ...
      expect(config.obsidianFormat).toBe(
        "[[Bible/{abbrev}/{chapter}#v{verse}]]",
      );
    });
  ...
  describe("formatObsidianLink", () => {
    ...
    test("should format with zero-padded chapter", () => {
      expect(
        formatObsidianLink("Gen", 1, 1, "[[{abbrev}-{chapter2}#v{verse}]]"),
      ).toBe("[[Gen-01#v1]]");
      expect(
        formatObsidianLink("Gen", 12, 1, "[[{abbrev}-{chapter2}#v{verse}]]"),
      ).toBe("[[Gen-12#v1]]");
      expect(
        formatObsidianLink("Ps", 119, 1, "[[{abbrev}-{chapter2}#v{verse}]]"),
      ).toBe("[[Ps-119#v1]]");
    });
    ...
    test("should handle templates without all placeholders", () => {
      expect(formatObsidianLink("Matt", 5, 3, "[[{abbrev}]]")).toBe("[[Matt]]");
      expect(formatObsidianLink("Matt", 5, 3, "[[Chapter {chapter}]]")).toBe(
        "[[Chapter 5]]",
      );
    });
  });
