...
describe("enrichment", () => {
  ...
  describe("Bible reference enrichment", () => {
    test("should enrich simple single verse reference", () => {
      ...
      expect(output).toContain(
        "[John 3:16](https://www.biblegateway.com/passage/?search=John%203%3A16&version=NRSVCE)",
      );
  ...
  describe("CCC reference enrichment", () => {
    test("should enrich simple CCC reference", () => {
      ...
      expect(output).toContain(
        "[CCC 528](https://www.catholiccrossreference.online/catechism/#!/search/528)",
      );
    });
...
`;
  ...
  describe("bare chapter references", () => {
    test("Isaiah 53 generates a Bible Gateway link and Obsidian chapter link", () => {
      ...
      expect(output).toContain(
        "[Isaiah 53](https://www.biblegateway.com/passage/?search=Isaiah%2053&version=NRSVCE)",
      );
    ...
    test("Psalm 91 generates a Bible Gateway link and Obsidian chapter link", () => {
      ...
      expect(output).toContain(
        "[Psalm 91](https://www.biblegateway.com/passage/?search=Psalm%2091&version=NRSVCE)",
      );
  ...
  describe("single-chapter books — bare verse references", () => {
    test("Jude 9 generates Bible Gateway link for Jude 1:9", () => {
      ...
      expect(output).toContain(
        "[Jude 9](https://www.biblegateway.com/passage/?search=Jude%201%3A9&version=NRSVCE)",
      );
    });
    ...
    test("Jude 9-14 generates Bible Gateway link and Obsidian range links", () => {
      ...
      expect(output).toContain(
        "[Jude 9-14](https://www.biblegateway.com/passage/?search=Jude%201%3A9-14&version=NRSVCE)",
      );
  ...
  describe("period, colon and ellipsis termination — bare chapter (pass 2c)", () => {
    ...
    test("Tobit 1. — period", () => {
      expect(enrichMarkdown("Tobit 1.")).toContain("[Tobit 1]");
    });
    test("Tobit 1: — colon", () => {
      expect(enrichMarkdown("Tobit 1:")).toContain("[Tobit 1]");
    });
    test("Tobit 1... — ellipsis", () => {
      expect(enrichMarkdown("Tobit 1...")).toContain("[Tobit 1]");
    });

    test("Judith 1. — period", () => {
      expect(enrichMarkdown("Judith 1.")).toContain("[Judith 1]");
    });
    test("Judith 1: — colon", () => {
      expect(enrichMarkdown("Judith 1:")).toContain("[Judith 1]");
    });
    test("Judith 1... — ellipsis", () => {
      expect(enrichMarkdown("Judith 1...")).toContain("[Judith 1]");
    });

    test("Wisdom 1. — period", () => {
      expect(enrichMarkdown("Wisdom 1.")).toContain("[Wisdom 1]");
    });
    test("Wisdom 1: — colon", () => {
      expect(enrichMarkdown("Wisdom 1:")).toContain("[Wisdom 1]");
    });
    test("Wisdom 1... — ellipsis", () => {
      expect(enrichMarkdown("Wisdom 1...")).toContain("[Wisdom 1]");
    });

    test("Sirach 1. — period", () => {
      expect(enrichMarkdown("Sirach 1.")).toContain("[Sirach 1]");
    });
    test("Sirach 1: — colon", () => {
      expect(enrichMarkdown("Sirach 1:")).toContain("[Sirach 1]");
    });
    test("Sirach 1... — ellipsis", () => {
      expect(enrichMarkdown("Sirach 1...")).toContain("[Sirach 1]");
    });

    test("Baruch 1. — period", () => {
      expect(enrichMarkdown("Baruch 1.")).toContain("[Baruch 1]");
    });
    test("Baruch 1: — colon", () => {
      expect(enrichMarkdown("Baruch 1:")).toContain("[Baruch 1]");
    });
    test("Baruch 1... — ellipsis", () => {
      expect(enrichMarkdown("Baruch 1...")).toContain("[Baruch 1]");
    });

    test("1 Maccabees 1. — period", () => {
      expect(enrichMarkdown("1 Maccabees 1.")).toContain("[1 Maccabees 1]");
    });
    test("1 Maccabees 1: — colon", () => {
      expect(enrichMarkdown("1 Maccabees 1:")).toContain("[1 Maccabees 1]");
    });
    test("1 Maccabees 1... — ellipsis", () => {
      expect(enrichMarkdown("1 Maccabees 1...")).toContain("[1 Maccabees 1]");
    });

    test("2 Maccabees 1. — period", () => {
      expect(enrichMarkdown("2 Maccabees 1.")).toContain("[2 Maccabees 1]");
    });
    test("2 Maccabees 1: — colon", () => {
      expect(enrichMarkdown("2 Maccabees 1:")).toContain("[2 Maccabees 1]");
    });
    test("2 Maccabees 1... — ellipsis", () => {
      expect(enrichMarkdown("2 Maccabees 1...")).toContain("[2 Maccabees 1]");
    });

    // Alternative names for Sirach
    test("Ecclesiasticus 1. — period", () => {
      expect(enrichMarkdown("Ecclesiasticus 1.")).toContain(
        "[Ecclesiasticus 1]",
      );
    });
    test("Wisdom of Ben Sira 1. — period", () => {
      expect(enrichMarkdown("Wisdom of Ben Sira 1.")).toContain(
        "[Wisdom of Ben Sira 1]",
      );
    });

    // Canonical examples from the issue
    test("Isaiah 53. — period", () => {
      expect(enrichMarkdown("Isaiah 53.")).toContain("[Isaiah 53]");
    });
    test("Isaiah 53: — colon", () => {
      expect(enrichMarkdown("Isaiah 53:")).toContain("[Isaiah 53]");
    });
    test("Isaiah 53... — ellipsis", () => {
      expect(enrichMarkdown("Isaiah 53...")).toContain("[Isaiah 53]");
    });
    test("Psalm 91. — period", () => {
      expect(enrichMarkdown("Psalm 91.")).toContain("[Psalm 91]");
    });
  ...
  describe("parenthetical references — issue #27", () => {
    // Pass 2 (BIBLE_REF_RE) — chapter:verse refs inside parens
    test("(Genesis 3:5) — parenthetical chapter:verse ref is enriched", () => {
      const output = enrichMarkdown("(Genesis 3:5)");
      expect(output).toContain("[Genesis 3:5]");
      expect(output).toContain("[[Gen-03#v5]]");
    });

    test("exact issue example: blockquote with parenthetical ref", () => {
      const input = "> *Ye shall be as gods* (Genesis 3:5)";
      const output = enrichMarkdown(input);
      expect(output).toContain(
        "[Genesis 3:5](https://www.biblegateway.com/passage/?search=Genesis%203%3A5&version=NRSVCE)",
      );
      expect(output).toContain("[[Gen-03#v5]]");
    });

    test("(John 3:16) — parenthetical NT ref is enriched", () => {
      const output = enrichMarkdown("(John 3:16)");
      expect(output).toContain("[John 3:16]");
      expect(output).toContain("[[John-03#v16]]");
    });

    // Pass 2b (SINGLE_CHAPTER_REF_RE) — single-chapter bare-verse refs inside parens
    test("(Jude 9) — parenthetical single-chapter bare-verse ref is enriched", () => {
      const output = enrichMarkdown("(Jude 9)");
      expect(output).toContain("[Jude 9]");
      expect(output).toContain("[[Jude#v9]]");
    });

    test("(Obadiah 21) — parenthetical single-chapter bare-verse ref is enriched", () => {
      const output = enrichMarkdown("(Obadiah 21)");
      expect(output).toContain("[Obadiah 21]");
    });

    // Pass 2c (BARE_CHAPTER_REF_RE) — bare chapter refs inside parens
    test("(Isaiah 53) — parenthetical bare chapter ref is enriched", () => {
      const output = enrichMarkdown("(Isaiah 53)");
      expect(output).toContain("[Isaiah 53]");
      expect(output).toContain("[[Isa-53]]");
    });

    test("(Psalm 91) — parenthetical bare chapter ref is enriched", () => {
      const output = enrichMarkdown("(Psalm 91)");
      expect(output).toContain("[Psalm 91]");
      expect(output).toContain("[[Ps-91]]");
    });

    // Idempotency — enriching already-enriched parenthetical output is a no-op
    test("idempotency: (Genesis 3:5) enriched twice equals enriched once", () => {
      const once = enrichMarkdown("(Genesis 3:5)");
      expect(enrichMarkdown(once)).toBe(once);
    });

    test("idempotency: (Jude 9) enriched twice equals enriched once", () => {
      const once = enrichMarkdown("(Jude 9)");
      expect(enrichMarkdown(once)).toBe(once);
    });

    test("idempotency: (Isaiah 53) enriched twice equals enriched once", () => {
      const once = enrichMarkdown("(Isaiah 53)");
      expect(enrichMarkdown(once)).toBe(once);
    });

    // Guard still active — already-linked refs inside parens are not double-enriched
    test("already-linked ref inside parens is not double-enriched", () => {
      const input =
        "([Genesis 3:5](https://www.biblegateway.com/passage/?search=Genesis%203%3A5&version=NRSVCE) ( [[Gen-03#v5]] ))";
      expect(enrichMarkdown(input)).toBe(input);
    });
  });
});
